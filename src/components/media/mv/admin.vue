...
<script>
import mvApi from '@/api/mv/index'
export default {
  name: "admin",
  data() {
    return {
  ...
  methods: {
    ...
    changeShowStatus(event, songId) {
      mvApi.changeShowStatus(songId, event).then(
        response => {
          if (response.code === 200) {
    ...
    page(queryVo) {
      mvApi.page(queryVo).then(
        response => {
          if (response.code === 200) {
            this.mvList = response.data.list
